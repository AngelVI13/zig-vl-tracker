...
fn parseDocument(ctx: *ParseContext, backing_allocator: *Allocator) !Document {
    ...
    var doc_allocator = doc.arena.allocator();

    doc.xml_decl = try tryParseProlog(ctx, &doc_allocator);
    _ = ctx.eatWs();
    doc.root = (try tryParseElement(ctx, &doc_allocator)) orelse return error.InvalidDocument;
    _ = ctx.eatWs();
